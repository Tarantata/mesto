...
//
const popupPerson = new Popup(popupTypePerson)
...
const image = new PopupWithImage(popupBigPictureImage);
image.setEventListeners();

const agent = new PopupWithForm()
...
const createCardElement = (item) => {
    const card = new Card({
        data: item,
        handleCardClick: (imageItem) => {
            image.open(imageItem)
        }
...
const cardArray = new Section({
    items: initialCards, // item будет элементом перебора массива
    renderer: (image) => {
        cardArray.addItem(createCardElement(image))
    }
}, cardContent);
